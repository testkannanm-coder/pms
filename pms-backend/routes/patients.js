...
router.get("/", authMiddleware, async (req, res) => {
  try {
    ...
    const patients = await patientService.getAllPatients(userId, filters);
...
router.get("/stats", authMiddleware, async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Failed to fetch patient statistics", error: error.message });
  }
});
...
router.put("/:id", authMiddleware, async (req, res) => {
  try {
    const userId = req.user.id;
    const patient = await patientService.updatePatient(req.params.id, userId, req.body);
